...
class PostmarkAdapter extends EmailProviderBase {
    ...
    async send(data, options) {
        ...
        try {
            ...
            if (e.code === 'POSTMARK_API_ERROR' || e.code === 'POSTMARK_NETWORK_ERROR') {
                ...
                this.#logging.error(`[Postmark Adapter] ${e.code}: ${e.message}`);
                this.#logging.error(`[Postmark Adapter] Error details: ${e.errorDetails || 'N/A'}`);
                this.#logging.error(`[Postmark Adapter] Context: ${e.context || 'N/A'}`);

                // Wrap as BULK_EMAIL_SEND_FAILED for proper handling by Ghost's batch service
    ...
    async fetchLatest(batchHandler, options) {
        ...
        // By default, look in the broadcast stream where bulk emails are sent
        const {configService} = this.config;
        const emailAdapterConfig = configService.get('adapters:email');
        const messageStream = emailAdapterConfig?.postmark?.messageStream || 'broadcast';
