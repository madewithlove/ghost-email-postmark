 ...
 *
 * Standalone email adapter for Ghost that handles both sending and analytics.
 * Implements the Ghost email adapter interface without requiring EmailProviderBase.
 */
        ...
        try {
            ...
            const replacements = replacementDefinitions.map((def) => {
                return {
                    id: def.id,
                    token: def.token,
                    // Create a global regexp from the token
                    regexp: new RegExp(def.token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')
                };
            });
