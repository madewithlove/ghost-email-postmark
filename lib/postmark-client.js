...
module.exports = class PostmarkClient {
    ...
    // Postmark traditional API supports up to 500 recipients per request
    static DEFAULT_BATCH_SIZE = 500;
    // Postmark Bulk API supports up to 500 messages per request
    static BULK_API_BATCH_SIZE = 500;
    // Bulk API payload size limit: 50MB
    static BULK_API_MAX_PAYLOAD_SIZE = 50 * 1024 * 1024;
    ...
    /**
     * Send emails using Postmark Bulk API with fallback to traditional API
     *
     ...
     * @param {Date} [message.deliveryTime] - Scheduled delivery time (not supported by Bulk API)
    ...
    async send(message, recipientData, replacements) {
        ...
        // Attempt Bulk API first (if available and no scheduled delivery)
        // Bulk API doesn't support scheduled delivery
        if (postmarkConfig.bulkApiEnabled && !message.deliveryTime) {
            try {
                return await this.#sendViaBulkApi(message, recipientData, replacements, postmarkConfig);
            } catch (error) {
                logging.warn('Postmark Bulk API failed, falling back to traditional API');
                logging.error(error);
                // Fall through to traditional API
            }
        }

        // Fallback to traditional API or if Bulk API is disabled
        return await this.#sendViaTraditionalApi(message, recipientData, replacements, postmarkConfig);
    }

    /**
     * Send via Postmark Bulk Email API
     *
     * @private
     */
    async #sendViaBulkApi(message, recipientData, replacements, postmarkConfig) {
        const startTime = Date.now();

        try {
            // Prepare template content with Postmark variable syntax
            let htmlBody = message.html;
            let textBody = message.plaintext;

            // Convert replacements to Postmark template syntax: {{VariableName}}
            replacements.forEach((replacement) => {
                const postmarkVariable = `{{${replacement.id}}}`;
                htmlBody = htmlBody.replace(replacement.regexp, postmarkVariable);
                if (textBody) {
                    textBody = textBody.replace(replacement.regexp, postmarkVariable);
                }
            });

            // Build messages array with individual recipient data
            const messages = Object.keys(recipientData).map((email) => {
                const templateModel = recipientData[email];
            ...
            const bulkRequest = {
                ...
                HtmlBody: htmlBody,
                MessageStream: postmarkConfig.messageStream || 'broadcasts'
            };

            if (textBody) {
                bulkRequest.TextBody = textBody;
            }

            if (message.replyTo) {
                bulkRequest.ReplyTo = message.replyTo;
            }

            if (message.track_opens) {
                bulkRequest.TrackOpens = true;
            }

            if (message.track_clicks) {
                bulkRequest.TrackLinks = 'HtmlAndText';
            }
            ...
            bulkRequest.Messages = messages;

            // Check payload size (rough estimate)
            const payloadSize = JSON.stringify(bulkRequest).length;
            if (payloadSize > PostmarkClient.BULK_API_MAX_PAYLOAD_SIZE) {
                throw new errors.IncorrectUsageError({
                    message: `Payload size (${(payloadSize / 1024 / 1024).toFixed(2)}MB) exceeds Postmark Bulk API limit (50MB)`
                });
            }

            debug(`[PostmarkClient] Sending via Bulk API to ${messages.length} recipients`);
            ...
            metrics.metric('postmark-send-mail-bulk', {
                value: Date.now() - startTime,
                statusCode: 200
            });

            logging.info(`[PostmarkClient] Sent via Bulk API (${Date.now() - startTime}ms)`);

            // Return the bulk request ID
            return {
                id: response.ID || response.SubmittedAt || `bulk-${Date.now()}`
            };
        } catch (error) {
            metrics.metric('postmark-send-mail-bulk', {
                value: Date.now() - startTime,
    ...
    /**
     * Send via traditional Postmark Email API (individual emails)
     *
     * @private
     */
    async #sendViaTraditionalApi(message, recipientData, replacements, postmarkConfig) {
        const startTime = Date.now();
    ...
    #getConfig() {
        ...
        // Check for Postmark config in bulkEmail.postmark
    ...
    getBatchSize() {
        ...
        // Use bulk API batch size if enabled, otherwise traditional API size
        if (config?.bulkApiEnabled) {
            return PostmarkClient.BULK_API_BATCH_SIZE;
        }

        return PostmarkClient.DEFAULT_BATCH_SIZE;
    }
