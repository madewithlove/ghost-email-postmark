...
module.exports = class PostmarkClient {
    ...
    async #sendViaBatchApi(message, recipientData, replacements, postmarkConfig) {
        ...
        try {
            ...
            debug(`[PostmarkClient] Sent via traditional API (${Date.now() - startTime}ms)`);
    ...
    async fetchEvents(postmarkOptions, batchHandler, {maxEvents = Infinity} = {}) {
        ...
        try {
            ...
            debug(`[PostmarkClient] Processed ${batchCount} batches in ${(totalDuration / 1000).toFixed(2)}s. Average batch time: ${(averageBatchTime / 1000).toFixed(2)}s`);
        } catch (error) {
    ...
    normalizeEvent(event, message = null, eventType = null) {
        ...
        if (!event.Type) {
            logging.error('[PostmarkClient] Received invalid event from Postmark');
            debug(`Invalid event: ${JSON.stringify(event)}`);
            return null;
        }
    ...
    async #makePostmarkRequest(endpoint, payload, config, method = 'POST') {
        ...
        debug(`[PostmarkClient] Making ${method} request to ${endpoint}`);
        ...
        try {
            ...
            if (method === 'POST' && payload) {
                ...
                debug(`[PostmarkClient] Payload size: ${JSON.stringify(payload).length} bytes, recipients: ${Array.isArray(payload) ? payload.length : 1}`);
            }

            debug(`[PostmarkClient] Sending request to ${url}`);
            ...
            try {
                ...
                debug(`[PostmarkClient] Response received with status: ${response.status}`);
            } catch (fetchError) {
                clearTimeout(timeoutId);
            ...
            debug(`[PostmarkClient] Request failed: ${error.message} (code: ${error.code || 'none'})`);

            // Re-throw if already a Ghost error (logged centrally by Ghost)
            if (error.code) {
                throw error;
            }
